...
if(isset($_POST['submit'])) {
    ...
    $query = "UPDATE teams set id = '$id', teamName = '$name', city = '$city' where id = '$id'";
    $resultInsert = mysqli_query($conn, $query);
    if ($resultInsert) {
        header('Location: teams.php?msg=Team updated');
    }else{
        echo "Failed updating team" . mysqli_error($conn);
    }
...
<form class="w-full max-w-lg justify-center items-center" method="post">
    <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full  px-3 mb-6 md:mb-0">
            ...
            <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text" name="name" value="<?php echo $row['teamName']?>">
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3">
            ...
            <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-age" type="text" name="city" value="<?php echo $row['city']?>">
        </div>
    </div>
    <button class="bg-black text-white p-5 rounded-md" name="submit" type="submit">Update Team</button>
</form>
</body>
</html>
